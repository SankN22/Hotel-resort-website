...
//   const { slug, email, price, size } = req.body;  
...
//   // execute the query
//   db.query(query,[slug, email, price, size], (err, results) => {
//     if (err) {
...
app.post('/book', (req, res) => {
  ...
  // create SQL query for inserting room data
  const insertQuery = "INSERT INTO room (slug, name, price, size) VALUES (?, ?, ?, ?)";
  const selectQuery = "SELECT count FROM availability WHERE room_name = ?";
  const updateQuery = "UPDATE availability SET count = count - 1 WHERE room_name = ?";

  // execute the insert query
  db.query(insertQuery, [slug, email, price, size], (err, results) => {
    if (err) {
      console.error('Error inserting room data: ', err);
      res.status(500).send('Error inserting room data.');
    } else {
      ...
      // execute the select query to check count
      db.query(selectQuery, [slug], (err, availabilityResults) => {
        if (err) {
          console.error('Error retrieving availability count: ', err);
          res.status(500).send('Error retrieving availability count.');
        } else {
          const count = availabilityResults[0].count;
          if (count > 0) {
            // execute the update query to decrement count
            db.query(updateQuery, [slug], (err, updateResults) => {
              if (err) {
                console.error('Error updating availability count: ', err);
                res.status(500).send('Error updating availability count.');
              } else {
                console.log('Availability count decremented successfully.');
                res.send('Room data inserted and availability count decremented successfully.');
              }
            });
          } else {
            res.send('Room data inserted successfully, but availability count is already 0.');
          }
        }
      });
    }
...
app.get('/bookings', (req, res) => {
  ...
  db.query(sql, (err, result) => {
    ...
    console.log("Booking queried successfully");
    res.json(result);
...
app.get('/bookings1', (req, res) => {
  ...
  db.query(sql, (err, result1) => {
    ...
    console.log("Room queried successfully");
    res.json(result1);
